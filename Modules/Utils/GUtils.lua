...
function GUtils:createDropdown(opts)
    ...
    local changeFunc = opts['changeFunc'] or function() end
    ...
    table.sort(itemsCopy, function(a, b) return string.len(a) > string.len(b) end)
...
function GUtils:createEditBox(opts)
    ...
    box.isValid = function()
        return box:IsVisible()
                ...
                or (box:GetText() and box:GetText() ~= ""))
    end
    box.getValue = function()
        if numeric then return box:GetNumber() end
        return box:GetText()
    end
    ...
    box:SetScript("OnTextChanged", function()
        ...
        if box.isValid() then textValidFunc() else textValidFunc() end
    end)
    ...
    if box:GetFrameLevel() > 4 then box_frame:SetFrameLevel(box:GetFrameLevel() - 4) end
...
function GUtils:createItemLink(parent)
    ...
    fs.SetItemLink = function(itemIdentifier)
        ...
        fs:SetText(itemLink)
        fs.iLink = itemLink
        if fs.icon then fs.icon:SetTexture(itemTexture) end
