...
function Loot:RegisterMobLoot()
    ...
    for i=1, GetNumLootItems() do
        ...
        local itemName = select(1, GetItemInfo(item)) or ''
        local itemLink = select(2, GetItemInfo(item)) or ''
        local itemId = select(2, strsplit(":", itemLink, 3))
        local itemRarity = select(3, GetItemInfo(item)) or ''
        local itemLevel = select(4, GetItemInfo(item)) or ''
        local itemReqLevel = select(5, GetItemInfo(item)) or ''
        local itemType = select(6, GetItemInfo(item)) or ''
        local itemSubType = select(7, GetItemInfo(item)) or ''
        local itemPrice = select(11, GetItemInfo(item)) or ''

        local text = itemName.."/"..itemRarity.."/"..itemLevel.."/"..itemReqLevel.."/"..itemType.."/"..itemSubType.."/"..itemPrice

        print(text)

        --if not tContains(Loot.mob_loot) then Loot.mob_loot[mob] = {} end
    end
